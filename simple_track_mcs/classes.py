...
class Track(object):
    ...
    def get_area(self):
        return [storm.area for storm in self.storms]
    ...
    def get_min_Tbs(self):
        return [storm.minTb for storm in self.storms]
    ...
    def is_any_in_region(self, region):
        """
        :param region: rgion dictionary, like  reg_SA = dict(lons=(13,35), lats=(-35,-22) )
        :return: Tracks that have a storm initiating in this region
        """
    ...
    def get_centroidLon(self):

        return [storm.centroidlon for storm in self.storms]
        
    def get_centroidLat(self):

        return [storm.centroidlat for storm in self.storms]
        
    def get_u(self):

        return [storm.u for storm in self.storms]
    
    def get_v(self):

        return [storm.v for storm in self.storms]
    
    def get_pr_mean(self):

        return [storm.pr.mean() for storm in self.storms]
    
    def get_pr_max(self):

        return [storm.pr.max() for storm in self.storms]
    
    def get_pr_min(self):

        return [storm.pr.min() for storm in self.storms]
    
    def get_pr_90perc(self):

        return [np.percentile(storm.pr, 90) for storm in self.storms]
